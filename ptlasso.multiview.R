ptLasso.multiview1.0 <- function(
  ...
  if (!is.null(fitoverall)) {
    if (!inherits(fitoverall, "cv.multiview"))
      stop("fitoverall must be a cv.multiview object.")
  }
  
  if (!is.null(fitind)) {
    if (length(fitind) != k)
      stop("fitind must contain one model per study.")
    if (!all(sapply(fitind, inherits, "cv.multiview")))
      stop("All elements of fitind must be cv.multiview objects.")
  }
  ...
  # =====================================================
  # STEP 8: Fit overall multiview model (cv.multiview.revised)
  ...
  if (is.null(fitoverall)) {
    ...
    fitoverall <- cv.multiview.revised(
      x_list         = x_list_all,
      y              = y_all,
      family         = family_fn(),
  ...
  if (is.null(fitind)) {
    ...
    for (kk in seq_len(k)) {
      ...
      fitind[[kk]] <- cv.multiview.revised(
        x_list = x[[kk]],
        y      = y[[kk]],
        foldid = foldid2[[kk]],
  ...
  if (alpha == 1) {
    ...
    for (kk in seq_len(k)) {
      ...
      fitpre[[kk]] <- cv.multiview.revised(
        x_list         = x[[kk]],
        y              = y[[kk]],
        foldid = foldid2[[kk]],
